...
import { Plus, Pencil, Trash2, Search, Inbox, Mail, Phone, ArrowRight, Building2, TrendingUp } from 'lucide-react';
...
const empty = { name: '', email: '', phone: '', project_type: 'Andet', description: '', estimated_budget: '', address: '', source: 'Forside kontakt', status: 'Ny', submitted_date: new Date().toISOString().split('T')[0], notes: '' };
...
export default function Kundehenvendelser() {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try { const payload = {...form, estimated_budget: Number(form.estimated_budget)||0}; if (editing) await base44.entities.WebsiteInquiry.update(editing.id, payload); else await base44.entities.WebsiteInquiry.create(payload); setDialogOpen(false); load(); }
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-3"><div><Label>Estimeret budget (DKK)</Label><Input type="number" value={form.estimated_budget} onChange={e=>field('estimated_budget',e.target.value)} /></div><div><Label>Kilde</Label><Select value={form.source} onValueChange={v=>field('source',v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.keys(sourceColors).map(c=><SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div></div>
