...
import { UserRound, HardHat, ListChecks, Save, Mail, Clock, MapPin, Calendar, Trash2, AlertTriangle } from 'lucide-react';
...
export default function Brugerprofil() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-1">Min profil</h1>
      <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Dine tildelte opgaver og aktive projekter</p>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 mb-6">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <div className="text-xl font-bold text-slate-900 dark:text-slate-100">{user.full_name || '—'}</div>
            <div className="text-sm text-slate-500 dark:text-slate-400 flex items-center gap-1.5 mt-1"><Mail className="w-3.5 h-3.5" /> {user.email}</div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4"><div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-xs mb-1"><HardHat className="w-3.5 h-3.5" /> Aktive projekter</div><div className="text-xl font-bold text-slate-900 dark:text-slate-100">{activeProjects.length}</div></div>
          <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4"><div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-xs mb-1"><ListChecks className="w-3.5 h-3.5" /> Åbne opgaver</div><div className="text-xl font-bold text-slate-900 dark:text-slate-100">{tasks.length}</div></div>
          <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4"><div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-xs mb-1"><Clock className="w-3.5 h-3.5" /> Timer i alt</div><div className="text-xl font-bold text-slate-900 dark:text-slate-100">{totalHours}t</div></div>
        </div>
      </div>

      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 mb-6">
        <h2 className="font-semibold text-slate-900 dark:text-slate-100 mb-4">Mine aktive projekter</h2>
            ...
            {activeProjects.map((p, i) => (
              <div key={i} className="bg-slate-50 dark:bg-slate-800/50 rounded-lg px-4 py-3">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2"><HardHat className="w-4 h-4 text-slate-400" /><span className="text-sm font-medium text-slate-700 dark:text-slate-300">{p.name}</span></div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 mb-6">
        <h2 className="font-semibold text-slate-900 dark:text-slate-100 mb-4">Mine opgaver</h2>
        {tasks.length === 0 ? <p className="text-sm text-slate-400">Ingen tildelte opgaver</p> : (
          <div className="space-y-2">
            {tasks.map((t) => (
              <div key={t.id} className="flex items-center justify-between bg-slate-50 dark:bg-slate-800/50 rounded-lg px-4 py-3">
                <div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6">
        <h2 className="font-semibold text-slate-900 dark:text-slate-100 mb-4">Personlige præferencer</h2>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-red-200 dark:border-red-900/50 p-6 mt-6">
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3 py-2">
            <p className="text-sm text-slate-600">
              For at bekræfte, skriv <span className="font-semibold text-slate-900 dark:text-slate-100">SLET</span> i feltet nedenfor:
            </p>
